...
namespace MariCommands.Sentry
{
    internal class MariCommandsEventProcessor : ISentryEventProcessor
    {
        ...
        public MariCommandsEventProcessor(IOptions<SentryMariCommandsOptions> options)
            => _options = options?.Value;

        public SentryEvent Process(SentryEvent @event)
        {
            // Move 'runtime' under key 'server-runtime' as User-Agent parsing done at
            // Sentry will represent the client's, from SentryAspNetCore.
            ...
            if (@event.ServerName == null)
            {
                ...
                // For ASP.NET Core apps, we always set the machine name if not explicitly set by the user, from SentryAspNetCore.
